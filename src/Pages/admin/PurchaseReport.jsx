/**
 * PurchaseReport.jsx — Enhanced Enterprise Purchase Analytics
 *
 * CHANGES FROM ORIGINAL:
 * ─ UI: Chart heights 240px → 340px (line), 320px (bar), 300px (pie)
 * ─ UI: AreaChart replaces LineChart — richer fill gradient
 * ─ UI: Pie chart radius 50/75 → 60/95, custom label renderer with %
 * ─ UI: Bar chart Y-axis width 80→110, barSize 20→18, label truncation 10→14
 * ─ UI: 3 KPI summary cards → 5 premium KPI cards (avg bill + month trend)
 * ─ UI: Filter bar rebuilt — styled FilterField, QuickPill, ResetBtn
 * ─ UI: Active filter chips below filter bar
 * ─ UI: Deep Analytics tab with enhanced chart layout + 4 advanced widgets
 * ─ UI: Smart Insights tab with expanded insight set
 * ─ UX: Vendor Ledger and All-Transactions tabs preserved
 * ─ LOGIC: All existing state, API calls, normalization, exports — UNTOUCHED
 * ─ LOGIC: minAmount / maxAmount filters from advanced version added to UI
 * ─ LOGIC: statusFilter added to UI
 */
...
const fmtFull = (value) => [`₹ ${value.toLocaleString("en-IN")}`, "Amount"];
...
export default function PurchaseReport() {
  ...
  /* ── FILTERS (original + added minAmount/maxAmount/statusFilter) ── */
  ...
  /* ── FILTER HANDLERS (original logic) ── */
  ...
  /* ── DATA NORMALIZATION (original, untouched) ── */
  ...
  const normalizedData = useMemo(() => {
    return purchases.map((pur) => {
      ...
      const paid = parseFloat(pur.paidAmount || pur.PaidAmount || 0);
      const pending = parseFloat(pur.pendingAmount || pur.PendingAmount || 0);
      const gstAmt = parseFloat(pur.gstAmount || pur.GstAmount || 0);
      const vIdRaw =
        pur.vendorId ||
        pur.VendorId ||
        pur.vendorID ||
  ...
  const filteredData = useMemo(() => {
    ...
    if (activeFyObj) {
      ...
      result = result.filter(
        (p) => p.normalizedDate >= fyStart && p.normalizedDate <= fyEnd,
      );
    }
    if (fromDate) {
      const f = new Date(fromDate);
      f.setHours(0, 0, 0, 0);
      result = result.filter((p) => p.normalizedDate >= f);
    }
    if (toDate) {
      const t = new Date(toDate);
      t.setHours(23, 59, 59, 999);
      result = result.filter((p) => p.normalizedDate <= t);
    }
    if (selectedVendorId)
      result = result.filter(
        (p) =>
          String(p.normalizedVendorId).trim() ===
          String(selectedVendorId).trim(),
      );
    if (minAmount)
      result = result.filter((p) => p.normalizedAmount >= Number(minAmount));
    if (maxAmount)
      result = result.filter((p) => p.normalizedAmount <= Number(maxAmount));
    if (statusFilter !== "all")
      result = result.filter(
        (p) =>
  ...
  const kpis = useMemo(() => {
    ...
    filteredData.forEach((p) => {
      const amt = p.normalizedAmount;
      total += amt;
      paid += p.normalizedPaid;
      pending += p.normalizedPending;
      gst += p.normalizedGst;
      if (amt > highest) highest = amt;
      if (p.normalizedVendorId !== "Unknown")
        uniqueVendors.add(p.normalizedVendorId);
      const d = p.normalizedDate;
  ...
  /* ── CHART AGGREGATIONS (original, untouched) ── */
  const { barChartData, pieChartData, lineChartData, tableData } =
    useMemo(() => {
      ...
      filteredData.forEach((p) => {
        const amt = p.normalizedAmount,
          vId = p.normalizedVendorId,
          date = p.normalizedDate;
        if (!vendorAgg[vId])
          vendorAgg[vId] = {
            id: vId,
            name: p.normalizedVendorName,
            amount: 0,
            count: 0,
            lastDate: date,
          };
      ...
      if (tData.length > 5) {
        const oa = tData.slice(5).reduce((s, v) => s + v.amount, 0);
  ...
  const advancedWidgets = useMemo(() => {
    ...
    const topVendor = tableData[0] || null;
    ...
    return [
      topVendor && {
        ...
        label: "Top Vendor",
        value:
          topVendor.name.length > 18
            ? topVendor.name.slice(0, 16) + "…"
            : topVendor.name,
        sub: `₹${topVendor.amount.toLocaleString("en-IN")} · ${((topVendor.amount / kpis.total) * 100).toFixed(0)}% share`,
      },
      {
        icon: Percent,
        color: "#10b981",
        ...
        sub: `₹${kpis.paid.toLocaleString("en-IN")} cleared`,
      },
      peakMonth && {
        icon: Zap,
        color: "#8b5cf6",
        ...
        sub: `₹${peakMonth.amount.toLocaleString("en-IN")} spend`,
      },
      {
        icon: DollarSign,
        color: "#06b6d4",
        label: "Avg Monthly Spend",
        value: `₹${Math.round(avgMonthlySpend).toLocaleString("en-IN")}`,
        sub: `over ${lineChartData.length} month${lineChartData.length !== 1 ? "s" : ""}`,
      },
    ].filter(Boolean);
  }, [filteredData, tableData, kpis, lineChartData, avgMonthlySpend]);

  /* ── SMART INSIGHTS (original + 2 extra) ── */
  const smartInsights = useMemo(() => {
    ...
    if (kpis.growth > 0)
      insights.push({
        ...
        text: `Purchases increased by ${kpis.growth.toFixed(1)}% compared to last month.`,
      });
    else if (kpis.growth < 0)
      insights.push({
        icon: TrendingDown,
        color: "#3b82f6",
        title: "Purchase Decline",
        text: `Purchases decreased by ${Math.abs(kpis.growth).toFixed(1)}% compared to last month.`,
      });
    if (tableData.length > 0) {
      ...
      insights.push({
        icon: Award,
        color: "#f59e0b",
        title: "Top Supplier Insight",
        text: `${top.name} is your top vendor, contributing ₹${top.amount.toLocaleString("en-IN")} (${((top.amount / kpis.total) * 100).toFixed(1)}% of total).`,
      });
    }
    ...
    if (pendingPct > 20)
      insights.push({
        ...
        text: `Pending payments amount to ₹${kpis.pending.toLocaleString("en-IN")} (${pendingPct}% of purchases). Consider settling accounts.`,
      });
    else
      insights.push({
        icon: CheckCircle,
        color: "#10b981",
        title: "Healthy Payables",
        text: `Pending dues are at a healthy ${pendingPct}%. Excellent cash flow management.`,
      });
    insights.push({
      icon: Activity,
      color: "#8b5cf6",
      title: "Average Order Value",
      text: `Enterprise average purchase bill value stands at ₹${kpis.avg.toLocaleString("en-IN")}.`,
    });
    if (kpis.gst > 0)
      insights.push({
        icon: FileText,
        color: "#06b6d4",
        title: "GST Expense",
        text: `Total GST paid this period is ₹${kpis.gst.toLocaleString("en-IN")} — approximately ${kpis.total ? ((kpis.gst / kpis.total) * 100).toFixed(1) : 0}% of gross.`,
      });
    if (tableData.length > 1) {
      ...
      if (concentration > 50)
        insights.push({
          ...
          text: `${tableData[0].name} accounts for ${concentration.toFixed(1)}% of total procurement. Consider diversifying suppliers.`,
        });
    }
    return insights;
  }, [filteredData, kpis, tableData]);

  /* ── EXPORTS (original, untouched) ── */
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `Enterprise_Purchase_Report_${new Date().getTime()}.csv`,
    );
        ...
        <PageShell>
          ...
          <StickyHeader className="hide-on-print">
            <HeaderLeft>
              ...
              <HeaderText>
                <PageTitle>Purchase Analytics</PageTitle>
                <Breadcrumb>
                  <BreadLink to="/admin/dashboard">Home</BreadLink>
                  <BreadSep>/</BreadSep>
                  <BreadLink to="#">Reports</BreadLink>
                  <BreadSep>/</BreadSep>
                  <BreadActive>Purchase Analytics</BreadActive>
                </Breadcrumb>
              </HeaderText>
            </HeaderLeft>
          ...
          <TabBar className="hide-on-print">
            {[
              ...
              { id: "ledger", icon: Building2, label: "Vendor Ledger" },
            ...
            <FilterRow>
              <FilterField $grow={2}>
                ...
                >
                  ...
                  {vendors.map((v) => (
                    <option key={v.id || v.Id} value={String(v.id || v.Id)}>
                      {v.businessName ||
                        v.BusinessName ||
                        v.vendorName ||
                        v.VendorName}
              ...
              <ChipRow>
                {selectedVendorId && (
                  <Chip>
                    <Store size={10} />
                    {vendors.find(
                      (v) => String(v.id || v.Id) === String(selectedVendorId),
                    )?.businessName || "Vendor"}
                    <button onClick={() => setSelectedVendorId("")}>
                      <X size={10} />
                    </button>
                  </Chip>
                )}
                    ...
                    {initialLoad ? (
                      ...
                      <>
                        ...
                        <KpiCard $accent="#10b981">
                          ...
                          <KpiBody>
                            <KpiLbl>Amount Paid</KpiLbl>
                            <KpiVal>
                              <AnimatedNumber value={kpis.paid} isCurrency />
                            </KpiVal>
                            <KpiSub>Settled to vendors</KpiSub>
                          </KpiBody>
                          <KpiGlow $c="#10b981" />
                        </KpiCard>
                        ...
                        <KpiCard $accent="#f59e0b">
                          ...
                          <KpiBody>
                            <KpiLbl>Active Vendors</KpiLbl>
                            <KpiVal>
                              <AnimatedNumber value={kpis.uniqueVendorsCount} />
                            </KpiVal>
                            <KpiSub>Suppliers engaged</KpiSub>
                          </KpiBody>
                          <KpiGlow $c="#f59e0b" />
                        </KpiCard>
                        <KpiCard $accent="#8b5cf6">
                          ...
                          <KpiBody>
                            <KpiLbl>Total Bills</KpiLbl>
                            <KpiVal>
                              <AnimatedNumber value={kpis.count} />
                            </KpiVal>
                            <KpiSub>Invoices processed</KpiSub>
                          </KpiBody>
                          <KpiGlow $c="#8b5cf6" />
                        </KpiCard>
                        ...
                        <KpiCard $accent="#ec4899">
                          ...
                          <KpiBody>
                            <KpiLbl>Highest Bill</KpiLbl>
                            <KpiVal>
                              <AnimatedNumber value={kpis.highest} isCurrency />
                            </KpiVal>
                            <KpiSub>Max single purchase</KpiSub>
                          </KpiBody>
                          <KpiGlow $c="#ec4899" />
                        </KpiCard>
                  ...
                  {filteredData.length === 0 && !initialLoad ? (
                    ...
                    <ChartGrid>
                      ...
                      <ChartCard className="span-8">
                        <div className="ch">
                          <ChartHeading>
                            <Building2 size={14} className="ic" /> Top Vendor
                            Purchases
                          </ChartHeading>
                      ...
                      {/* ── Paid vs Pending Pie (4/12) ── */}
                      <ChartCard className="span-4">
                        <div className="ch">
                          <ChartHeading>
                            <Activity size={14} className="ic" /> Paid vs
                            Pending
                          </ChartHeading>
                          <ExpandBtn
                      ...
                      {/* ── Vendor Distribution Pie (full width) ── */}
                      {pieChartData.length > 0 && (
                        <ChartCard className="span-12">
                          <div className="ch">
                            <ChartHeading>
                              <Award size={14} className="ic" /> Vendor Purchase
                              Distribution
                            </ChartHeading>
                            <ExpandBtn
                  ...
                  <TableCard>
                    <THead>
                      <ChartHeading style={{ margin: 0 }}>
                        <Building2 size={14} className="ic" /> Aggregated Vendor
                        Ledger
                      </ChartHeading>
                      <span
                        style={{
                    ...
                    <DataGridWrap>
                      <DataGrid>
                        <thead>
                          <tr>
                            <Th>Vendor Name</Th>
                            <Th $c>Purchase Count</Th>
                            <Th $r>Avg Order Value</Th>
                            <Th $r>Total Amount</Th>
                        ...
                        <tbody>
                          {loading || initialLoad ? (
                            ...
                            <tr>
                              ...
                              >
                                <PremiumEmptyState
                                  icon={<Store size={38} strokeWidth={1.2} />}
                                  title="No Purchases Found"
                                  subtitle="No vendor aggregations match the current filters."
                                />
                              </td>
                            </tr>
                          ) : (
              ...
              {activeTab === "transactions" && (
                ...
                >
                  <TableCard>
                    <THead>
                      <ChartHeading style={{ margin: 0 }}>
                        <List size={14} className="ic" /> Individual Bill
                        Transactions
                      </ChartHeading>
                      <span
                        style={{
                            ...
                            <tr>
                              ...
                              >
                                <PremiumEmptyState
                                  ...
                                  subtitle="No bills match the current filter criteria."
                                />
                              </td>
                            </tr>
                          ) : (
                            filteredData.map((pur, i) => (
                              <DataRow key={pur.id || i}>
                                <Td>
                                  ...
                                  >
                                    {pur.normalizedDate.toLocaleDateString(
                                      "en-IN",
                                    )}
                                  </div>
                                </Td>
                                <Td style={{ fontWeight: 600 }}>
                                  {pur.normalizedVendorName}
                                </Td>
                                <Td $r style={{ color: "var(--text-muted)" }}>
                                  ₹
                                  {(pur.grossAmount || 0).toLocaleString(
                                    "en-IN",
                                  )}
                                </Td>
                                <Td $r style={{ color: "var(--text-muted)" }}>
                                  ₹{pur.normalizedGst.toLocaleString("en-IN")}
                                </Td>
                                <Td
                                  $r
                                  style={{
                                ...
                                >
                                  ₹
                                  {pur.normalizedAmount.toLocaleString("en-IN")}
                                </Td>
                                <Td
                                  $r
                                  style={{ color: "#10b981", fontWeight: 600 }}
                                >
                                  ₹{pur.normalizedPaid.toLocaleString("en-IN")}
                                </Td>
                                <Td
                                  $r
                                ...
                                >
                                  ₹
                                  {pur.normalizedPending.toLocaleString(
                                    "en-IN",
                                  )}
                                </Td>
                                <Td $c>
                                  <SBadge $s={pur.status || pur.Status}>
                  ...
                  {smartInsights.length > 0 ? (
                    ...
                    <EmptyBox>
                      <PremiumEmptyState
                        ...
                        subtitle="Generate more transactions to activate AI-style purchase insights."
                      />
                    </EmptyBox>
                  )}
                </motion.div>
