...
const CHART_COLORS = ["#3b82f6", "#06b6d4", "#10b981", "#8b5cf6", "#f59e0b", "#ec4899"];
...
export default function SalesReport() {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      if (fyRes?.status === "OK" && Array.isArray(fyRes.result)) {
        ...
        const activeFy = fyRes.result.find((y) => y.isActive && !y.isDelete) || fyRes.result[0];
        if (activeFy) {
      ...
      // Fetch Clients & Invoices concurrently
  ...
  const resetFilters = () => {
    ...
    setToDate("");
    setQuickFilter("all");
    const activeFy = financialYears.find((y) => (y.isActive || y.IsActive) && !(y.isDelete || y.IsDelete));
  ...
  const activeFyObj = useMemo(
    () => financialYears.find((fy) => (fy.id || fy.Id)?.toString() === selectedFyId?.toString()),
  ...
  const clientMap = useMemo(() => {
    ...
    clients.forEach((c) => {
      const cId = c.id || c.Id;
      map[cId] = c.businessName || c.BusinessName || c.clientName || c.ClientName || `Client #${cId}`;
    });
    return map;
  }, [clients]);
  ...
  const normalizedData = useMemo(() => {
    return invoices.map(inv => {
      ...
      const rawDate = inv.invoiceDate || inv.InvoiceDate || inv.createdAt || inv.CreatedAt;
      const date = rawDate ? new Date(rawDate) : new Date();
      
      const amount = parseFloat(
        inv.total || inv.Total || 
        inv.totalAmount || inv.TotalAmount || 
        inv.grandTotal || inv.GrandTotal || 
        inv.netAmount || inv.NetAmount || 0
      );

      const clientId = inv.clientId || inv.ClientId || inv.clientMasterId || inv.ClientMasterId || inv.clientName || inv.ClientName || "Unknown";
      const clientName = clientMap[clientId] || inv.clientName || inv.ClientName || "Unknown Client";

      return {
        ...inv,
        normalizedDate: date,
        normalizedAmount: amount,
        normalizedClientId: clientId.toString(),
  ...
  const filteredData = useMemo(() => {
    ...
    if (activeFyObj) {
      ...
      result = result.filter((inv) => {
        return inv.normalizedDate >= fyStart && inv.normalizedDate <= fyEnd;
      });
    }
    ...
    if (fromDate) {
      ...
      result = result.filter((inv) => inv.normalizedDate >= from);
    }
    if (toDate) {
      const to = new Date(toDate);
      to.setHours(23, 59, 59, 999);
      result = result.filter((inv) => inv.normalizedDate <= to);
    }

    // 3. Exact Client Filter
    if (selectedClientId) {
      result = result.filter((inv) => inv.normalizedClientId === selectedClientId.toString());
    }

    return result;
  }, [normalizedData, activeFyObj, fromDate, toDate, selectedClientId]);

  // 🌟 AGGREGATIONS FOR DASHBOARD CARDS
  const { totalRevenue, totalInvoices, uniqueClientsCount } = useMemo(() => {
    let rev = 0;
    const uniqueClients = new Set();
    
    filteredData.forEach((inv) => {
      rev += inv.normalizedAmount;
      if (inv.normalizedClientId && inv.normalizedClientId !== "Unknown") {
  ...
  const { barChartData, pieChartData, lineChartData, tableData } = useMemo(() => {
    const clientAgg = {};
    const monthAgg = {};

    filteredData.forEach((inv) => {
      const amt = inv.normalizedAmount;
      const cId = inv.normalizedClientId;
      const date = inv.normalizedDate;
      
      // Client Mapping for Table & Bar/Pie Charts
      if (!clientAgg[cId]) {
        clientAgg[cId] = {
          id: cId,
      ...
      if (!monthAgg[monthKey]) {
        monthAgg[monthKey] = { sortKey: monthKey, month: monthLabel, revenue: 0 };
      }
      monthAgg[monthKey].revenue += amt;
    });
    ...
    const tData = Object.values(clientAgg)
      .map(c => ({
        ...c,
        avgOrderValue: c.count > 0 ? c.revenue / c.count : 0
      }))
    ...
    const pData = tData.slice(0, 5).map(c => ({ name: c.name, value: c.revenue })); 
    if (tData.length > 5) {
      const othersRev = tData.slice(5).reduce((sum, c) => sum + c.revenue, 0);
      if(othersRev > 0) pData.push({ name: "Others", value: othersRev });
    }

    // Monthly Data Sorted By Time
    const lData = Object.values(monthAgg)
      .sort((a, b) => a.sortKey.localeCompare(b.sortKey));

    return { barChartData: bData, pieChartData: pData, lineChartData: lData, tableData: tData };
  }, [filteredData]);

  // 📤 EXPORTS
  const handleExportCSV = () => {
    if (tableData.length === 0) return errorAlert("No Data", "No data to export");
    const headers = ["Client Name,Invoice Count,Total Revenue (INR),Avg Order Value (INR),Last Transaction Date"];
    const rows = tableData.map(
      (r) =>
        `"${r.name.replace(/"/g, '""')}",${r.count},${r.revenue.toFixed(2)},${r.avgOrderValue.toFixed(2)},"${new Date(
          r.lastDate
        ).toLocaleDateString()}"`
    );
  ...
  const handleExportPDF = () => {
    ...
    const fyName = activeFyObj ? (activeFyObj.yearName || activeFyObj.YearName) : "All Time";
    const opt = {
      margin: [10, 10, 10, 10],
  ...
  const activeFiltersCount = [selectedClientId, fromDate, toDate].filter(Boolean).length;
        ...
        <PageWrapper className="p-2 p-md-3">
          ...
          <HeaderSection className="mb-3 hide-on-print">
            <div className="title-area">
              <PageTitle>
                <ChartBar className="title-icon" size={24} /> Sales Report
              </PageTitle>
              <p className="subtitle">Enterprise revenue analytics & client performance</p>
            </div>
                  ...
                  {exportMenuOpen && (
                    ...
                    >
                      <DropdownItem onClick={handleExportPDF}>
                        <FileText size={14} className="text-danger" /> Download PDF
                      </DropdownItem>
                      <DropdownItem onClick={handleExportCSV}>
                        <FileDown size={14} className="text-success" /> Download CSV
                      </DropdownItem>
                      <div className="dropdown-divider my-1 border-custom"></div>
                      {/* <DropdownItem onClick={handlePrint}>
                        <Printer size={14} className="text-primary" /> Print Layout
                      </DropdownItem> */}
                    </DropdownMenu>
                  )}
                </AnimatePresence>
          ...
          <FilterWrapper className="hide-on-print mb-3">
            <QuickFilters>
              <button className={quickFilter === "today" ? "active" : ""} onClick={() => handleQuickFilter("today")}>Today</button>
              <button className={quickFilter === "month" ? "active" : ""} onClick={() => handleQuickFilter("month")}>This Month</button>
              <button className={quickFilter === "year" ? "active" : ""} onClick={() => handleQuickFilter("year")}>This Year</button>
              <button className={quickFilter === "all" ? "active" : ""} onClick={() => handleQuickFilter("all")}>All Time</button>
            </QuickFilters>

            <CompactFilterBar>
              <div className="filter-item flex-grow-1" style={{ minWidth: "180px" }}>
                ...
                >
                  ...
                  {clients.map((c) => (
                    <option key={c.id || c.Id} value={c.id || c.Id}>
                      {c.businessName || c.BusinessName || c.clientName || c.ClientName}
                    </option>
                  ))}
                </select>
              </div>
                ...
                >
                  ...
                  {financialYears.map((fy) => (
                    <option key={fy.id || fy.Id} value={fy.id || fy.Id}>
                      {fy.yearName || fy.YearName} {(fy.isActive || fy.IsActive) ? "★" : ""}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <button className="btn-reset position-relative" onClick={resetFilters}>
          ...
          <div ref={reportRef} id="report-content-to-export" className="print-wrapper">
            ...
            <PrintHeader className="show-on-print-only">
              <div className="d-flex align-items-center justify-content-between mb-2 pb-2 border-bottom">
                <div>
                  ...
                  <p className="m-0 text-muted" style={{fontSize: '12px'}}>ERP Billing System Output</p>
                </div>
              ...
              <div className="print-meta">
                ...
                {activeFyObj && <span>FY: {activeFyObj.yearName || activeFyObj.YearName}</span>}
            ...
            <FyBadgeWrapper className="hide-on-print">
              {activeFyObj ? (
                <FyBadge>
                  <Calendar size={12} /> DATA FILTERED BY FY: {activeFyObj.yearName || activeFyObj.YearName}
                </FyBadge>
              ) : (
                ...
                <>
                  <SummaryCard>
                    <div className="inner-content">
                      <div className="d-flex justify-content-between align-items-start mb-2">
                        <span className="text-muted-custom fw-semibold text-uppercase tracking-wide" style={{ fontSize: "10px" }}>
                          Total Revenue
                        </span>
                      ...
                      <h3 className="fw-bold mt-1 text-custom mb-0 fs-4 text-gradient">
                        <AnimatedNumber value={totalRevenue} isCurrency={true} />
                      </h3>
                      <small className="text-success mt-1 d-block fw-bold d-flex align-items-center gap-1" style={{ fontSize: "10px" }}>
                        <ArrowUpRight size={12} /> Total Accrued Value
                      </small>
                    </div>
                  </SummaryCard>
                  ...
                  <SummaryCard>
                    <div className="inner-content">
                      ...
                      <small className="text-muted-custom mt-1 d-block fw-bold d-flex align-items-center gap-1" style={{ fontSize: "10px" }}>
                  ...
                  <SummaryCard>
                    <div className="inner-content">
                      ...
                      <small className="text-muted-custom mt-1 d-block fw-bold" style={{ fontSize: "10px" }}>
                        Unique transacting accounts
                      </small>
                    </div>
                  </SummaryCard>
                </>
            ...
            {!initialLoad && tableData.length > 0 && (
              <ChartGrid className="mb-3">
                <GlassCard className="p-3 chart-line">
                  <h6 className="fw-bold text-custom mb-3 d-flex align-items-center gap-2" style={{ fontSize: "12px", textTransform: "uppercase" }}>
                    <TrendingUp size={14} className="text-primary" /> Monthly Revenue Trend
                  </h6>
                  <div style={{ height: "240px", width: "100%" }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={lineChartData} margin={{ top: 5, right: 20, left: 0, bottom: 5 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="var(--border-custom)" opacity={0.3} vertical={false} />
                        <XAxis dataKey="month" stroke="var(--text-muted)" fontSize={10} tickLine={false} axisLine={false} />
                        <YAxis stroke="var(--text-muted)" fontSize={10} tickLine={false} axisLine={false} tickFormatter={(val) => `₹${(val / 1000).toFixed(0)}k`} />
                        <RechartsTooltip
                          contentStyle={{ background: "var(--card)", border: "1px solid var(--border-custom)", borderRadius: "8px", fontSize: "11px", boxShadow: "0 4px 15px rgba(0,0,0,0.1)" }}
                          itemStyle={{ color: "var(--primary)", fontWeight: "bold" }}
                          formatter={(value) => [`₹ ${value.toLocaleString("en-IN")}`, "Revenue"]}
                        />
                        <Line type="monotone" dataKey="revenue" stroke="#3b82f6" strokeWidth={3} dot={{ r: 3, fill: "#3b82f6", strokeWidth: 2, stroke: "var(--card)" }} activeDot={{ r: 6, fill: "#06b6d4", stroke: "#fff" }} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                </GlassCard>
                ...
                <GlassCard className="p-3 chart-bar">
                  <h6 className="fw-bold text-custom mb-3 d-flex align-items-center gap-2" style={{ fontSize: "12px", textTransform: "uppercase" }}>
                    <ChartBar size={14} className="text-primary" /> Top Client Revenue
                  </h6>
                  <div style={{ height: "240px", width: "100%" }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={barChartData} margin={{ top: 5, right: 10, left: -20, bottom: 5 }} layout="vertical">
                        <CartesianGrid strokeDasharray="3 3" stroke="var(--border-custom)" opacity={0.3} horizontal={false} />
                        <XAxis type="number" stroke="var(--text-muted)" fontSize={10} tickLine={false} axisLine={false} tickFormatter={(val) => `₹${(val / 1000).toFixed(0)}k`} />
                        <YAxis type="category" dataKey="name" stroke="var(--text-muted)" fontSize={10} tickLine={false} axisLine={false} width={80} tickFormatter={(val) => val.length > 10 ? val.substring(0, 10) + ".." : val} />
                        <RechartsTooltip
                          cursor={{ fill: "var(--bg-hover)" }}
                          contentStyle={{ background: "var(--card)", border: "1px solid var(--border-custom)", borderRadius: "8px", fontSize: "11px" }}
                ...
                <GlassCard className="p-3 chart-pie hide-on-print">
                  <h6 className="fw-bold text-custom mb-3 d-flex align-items-center gap-2" style={{ fontSize: "12px", textTransform: "uppercase" }}>
                    <PieChartIcon size={14} className="text-primary" /> Revenue Distribution
                  </h6>
            ...
            <GlassCard className="p-0 overflow-hidden print-no-border mb-4">
              <div className="p-3 border-bottom border-custom hide-on-print">
                <h6 className="fw-bold text-custom m-0 d-flex align-items-center gap-2" style={{ fontSize: "12px", textTransform: "uppercase" }}>
                  <FileText size={14} className="text-primary" /> Detailed Client Ledger
                </h6>
              </div>
              <TableWrapper>
                      ...
                      <tr>
                        <td colSpan="5" className="p-5 border-0">
                          <PremiumEmptyState
                            ...
                            subtitle="No invoices match the current date and filter criteria."
                          />
                        </td>
                      </tr>
                    ) : (
                        ...
                        <tr key={i} className="list-row">
                          <td className="fw-semibold text-custom py-3">
                            <div className="d-flex align-items-center gap-2">
                              <div className="avatar-sm bg-primary-subtle text-primary rounded d-flex align-items-center justify-content-center fw-bold hide-on-print" style={{width: '24px', height: '24px', fontSize: '10px'}}>
                          ...
                          <td className="text-right text-muted-custom py-3">
                            ₹ {row.avgOrderValue.toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                          </td>
                          ...
                          <td className="text-right text-muted-custom small py-3">
                            {new Date(row.lastDate).toLocaleDateString("en-IN", { day: '2-digit', month: 'short', year: 'numeric' })}
                          </td>
                        </tr>
                      ))
                    )}
...
const PrintHeader = styled.div`
  h2 { font-size: 20px; font-weight: bold; margin: 0 0 4px 0; color: #0f172a; }
  .print-meta { display: flex; gap: 20px; font-size: 11px; color: #64748b; font-weight: 600; margin-top: 10px; }
`;
...
const CompactFilterBar = styled.div`
  ...
  .filter-item {
    ...
    &:focus-within { border-color: var(--primary); box-shadow: 0 0 0 2px rgba(59, 130, 246, 0.1); }

    .icon { color: var(--text-muted); margin-right: 8px; }
...
const ChartGrid = styled.div`
  ...
  .chart-line { grid-column: span 12; }
  .chart-bar { grid-column: span 8; }
  .chart-pie { grid-column: span 4; }

  @media (max-width: 992px) {
    .chart-bar, .chart-pie { grid-column: span 12; }
  }
`;
...
const Table = styled.table`
  ...
  tr.list-row { transition: all 0.2s ease; background: transparent; }
  tr.list-row:hover { background: var(--bg-light-custom); }
